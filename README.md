...
## 🛠 1. 스킬 시스템 — 40개 스킬을 공통된 틀로 확장하기

### 무엇을
6개 캐릭터별 **총 40개의 액티브 스킬**과 **레벨 10/20/30 단계별 강화 효과**를 구현했습니다.

### 왜
- 스킬마다 발동 방식(논타겟 / 가장 가까운 적 / 다중 타겟), 지속 형태(즉발 / 범위 지속 / 투사체)가 모두 달라, 매번 분기 처리할 수 없음.
...
## 🛠 2. 상태이상 시스템 — 9종 효과를 하나의 매니저로 관리

### 무엇을
기절 / 공중부양 / 넉백 / 빙결 / 화상 / 출혈 / 공격력↑ / 크리티컬↑ / 가속 — **9가지 상태이상**을 구현하고, **중첩시간 갱신·만료 처리**를 일관되게 관리하는 매니저를 만들었습니다.

### 왜
- 같은 적에게 화상 + 출혈 + 빙결이 동시에 들어올 때, 각 효과마다 시간 갱신·중첩 가능 여부가 다 다름.
- 효과별로 코드를 흩뿌리면 **중복 적용 / 해제 누락 / 파티클이 안 사라지는 문제** 등 버그가 생김.

### 어떻게
`StatusEffect` 추상 클래스를 정의하고, 각 효과를 동일 인터페이스로 추상화.
...
- **단일 진입점** : `AddBuff()` 하나로 9종 효과 모두 처리. 매니저는 구체적인 타입을 몰라도 됨.
-  **중첩 여부를 데이터로 관리** : 각 효과가 자기 `isDuplicated` 값만 가지고 있으면, 매니저는 그걸 보고 누적할지 새로 추가할 지 판단.
- **뒷정리 자동화** : 코루틴 종료 시 `Remove()` → 파티클 풀 반환 → 리스트 제거가 자동으로 흐름.
